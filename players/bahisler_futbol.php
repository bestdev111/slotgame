...
$sor = sed_sql_query("select * from maclar_oranlar where mac_db_id='".$_POST['macdbid']."' and spor_tipi='futbol' and oran_val_id='".$key."' and bayi_id='".$ub['id']."'");

if(sed_sql_numrows($sor)<1) {

sed_sql_query("INSERT INTO maclar_oranlar SET mac_db_id='".$_POST['macdbid']."', spor_tipi='futbol', oran_val_id='".$key."', bayi_id = '".$ub['id']."', oran = '".$value."'");

} else {

sed_sql_query("update maclar_oranlar set oran = '".$value."' where mac_db_id='".$_POST['macdbid']."' and spor_tipi='futbol' and oran_val_id='".$key."' and bayi_id='".$ub['id']."'");
...
$sor = sed_sql_query("select mac_db_id,oran_tip,durum,oran_val_id from oranlar_futbol where mac_db_id='$mac_id' $gizli_ekle and durum='1' group by oran_tip order by oran_tip asc");
...
<tbody>
<tr>
<th class="l"> <a class="tipS" title="<?=getTranslation('foran'.$tip['id'].'')?>"><img src="/resources/themes/common/icons/16/info.png" alt="" align="absmiddle" style="margin-right:15px;"></a><?=getTranslation('foran'.$tip['id'].'')?></th>
...
<?
$sss = sed_sql_query("select mac_db_id,oran_tip,oran_val_id,oran,yapi,durum from oranlar_futbol ora where mac_db_id='$mac_id' and durum='1' and oran_tip='$ass[oran_tip]' $gizli_ekle order by (select yapi from oran_val_futbol ov where ov.id=ora.oran_val_id) asc");
while($row=sed_sql_fetchassoc($sss)) { 
$oran_bilgi = bilgi("select id,oran_val,yapi,oran_tip from oran_val_futbol where id='$row[oran_val_id]'");
$buoran = $row['oran'];
$oran_bilgi_2 = bilgi("select oran from maclar_oranlar where oran_val_id='".$oran_bilgi['id']."' and mac_db_id='".$mac_id."' and spor_tipi='futbol' and bayi_id='".$ub['id']."'");
