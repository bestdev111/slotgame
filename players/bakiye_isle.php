...
header("Location:bakiye_isle.php?onay=1");//BUNA YETKİLİ DEĞİLSİNİZ !

} else if($tip=='ekle') {
	
	if($ub["bakiye"]<$tutar){
	
		header("Location:bakiye_isle.php?onay=6");//KREDİ EKLENMEMİŞTİR !

	} else {
	
		sed_sql_query("update kullanici set bakiye=bakiye-$tutar where id='$ub[id]'");
	
		sed_sql_query("INSERT INTO hesap_hareket SET user_id='".$ub['id']."',username='".$ub['username']."',tip='cikar',tutar='".$tutar."',aciklama='Müşteriye Aktarılan Bakiye',islemi_yapan='".$ub['username']."',zaman='".time()."',onceki_tutar='".$ub['bakiye']."'");
		
		sed_sql_query("update kullanici set bakiye=bakiye+$tutar where id='$userid'");
	
		sed_sql_query("INSERT INTO hesap_hareket SET user_id='".$userid."',username='".$ouser['username']."',tip='ekle',tutar='".$tutar."',aciklama='Hesaptan Müşteriye',islemi_yapan='".$ub['username']."',zaman='".time()."',onceki_tutar='".$ouser['bakiye']."'");
	
		header("Location:bakiye_isle.php?onay=2");//KREDİ EKLENMİŞTİR.
	...
	if($ouser['bakiye']<$tutar){ 
		...
		header("Location:bakiye_isle.php?onay=5");//ÇIKARTILACAK BAKİYE HATALI.
	
	} else {
		
		sed_sql_query("update kullanici set bakiye=bakiye+$tutar where id='$ub[id]'");
	
		sed_sql_query("INSERT INTO hesap_hareket SET user_id='".$ub['id']."',username='".$ub['username']."',tip='ekle',tutar='".$tutar."',aciklama='Müşteriden Çekilen Bakiye',islemi_yapan='".$ub['username']."',zaman='".time()."',onceki_tutar='".$ub['bakiye']."'");

		sed_sql_query("update kullanici set bakiye=bakiye-$tutar where id='$userid'");
		
		sed_sql_query("INSERT INTO hesap_hareket SET user_id='".$userid."',username='".$ouser['username']."',tip='cikar',tutar='".$tutar."',aciklama='Müşteriden Hesaba',islemi_yapan='".$ub['username']."',zaman='".time()."',onceki_tutar='".$ouser['bakiye']."'");

		header("Location:bakiye_isle.php?onay=3");//KREDİ ÇIKARTILMIŞTIR.
	
	}

} else {

	header("Location:bakiye_isle.php?onay=4");//İŞLEM SEÇİNİZ !
...
<? if($_GET['onay']==1) { ?>
<div class="infomsg" onclick="javascript:$(this).hide();"><?=getTranslation('uyaribakiyeisle1')?></div>
<? } else if($_GET['onay']==2) { ?>
<div class="infomsg" onclick="javascript:$(this).hide();"><?=getTranslation('uyaribakiyeisle2')?></div>
<? } else if($_GET['onay']==3) { ?>
<div class="infomsg" onclick="javascript:$(this).hide();"><?=getTranslation('uyaribakiyeisle3')?></div>
<? } else if($_GET['onay']==4) { ?>
<div class="infomsg" onclick="javascript:$(this).hide();"><?=getTranslation('uyaribakiyeisle4')?></div>
<? } else if($_GET['onay']==5) { ?>
<div class="infomsg" onclick="javascript:$(this).hide();"><?=getTranslation('uyaribakiyeisle5')?></div>
<? } else if($_GET['onay']==6) { ?>
<div class="infomsg" onclick="javascript:$(this).hide();"><?=getTranslation('uyaribakiyeisle6')?></div>
<? } ?>
...
<li><?=getTranslation('aciklamabakiyeisle1')?></li>
<li><?=getTranslation('aciklamabakiyeisle2')?></li>
<li><?=getTranslation('aciklamabakiyeisle3')?></li>
<li><?=getTranslation('aciklamabakiyeisle4')?></li>
</ul>
</div>
</td>
</tr>
...
<thead>
<tr>
<th colspan="8"><?=getTranslation('bakiyeislegerceklesentransfer')?></th>
</tr>
<tr>
<th>#</th>
<th><?=getTranslation('bakiyeislekullanici')?></th>
<th><?=getTranslation('bakiyeisleislem')?></th>
<th><?=getTranslation('bakiyeisleeskikredi')?></th>
<th><?=getTranslation('bakiyeislemiktar')?></th>
<th><?=getTranslation('bakiyeisleyenikredi')?></th>
<th><?=getTranslation('bakiyeisletarih')?></th>
<th><?=getTranslation('bakiyeisleeylem')?></th>
</tr>
</thead>
<tbody>
