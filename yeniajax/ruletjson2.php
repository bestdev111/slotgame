...
$sor_rulet = sed_sql_query("select kupon_id,roundid,sayi,sonuc,id from kupon_ic_rulet where rulet_tip='2' and kazanma='1' and sonuc!='' and sonuczamani!=''");
while($row_rulet=sed_sql_fetcharray($sor_rulet)){
echo "RULET 2 - $row_rulet[kupon_id] - Nolu Kupon Sonuçlandırıldı. *** ";
...
$bilgile = bilgi("select * from kupon_ic_rulet where rulet_tip='2' and oddid='$runid' and sonuc='' and sonuczamani='' and kazanma='1'");
if($bilgile['id'] > 0){
sed_sql_query("UPDATE kupon_ic_rulet SET sonuc = '".$sonuc."',sonuczamani = '".$sonuczamani_ver."' WHERE oddid='".$runid."' and rulet_tip='2'");
}
...
$sor = sed_sql_query("select * from kupon_ic_rulet where kupon_id='$sonkupon[id]' and rulet_tip='2'");
...
('','$session_id','$row[gameid]','$row[roundid]','$row[grupid]','$row[oran]','$row[secenek]','$row[sayi]','2','$suan')");

}

$kontrol_kupon = sed_sql_query("select * from kupon_rulet where session_id='$session_id' and rulet_tip='2' order by id");
...
$kontrol = sed_sql_query("select * from kupon_rulet where session_id='$session_id' and rulet_tip='2'");
...
sed_sql_query("delete from kupon_rulet where session_id='".$session_id."' and rulet_tip='2'");
...
sed_sql_query("delete from kupon_rulet where session_id='".$session_id."' and rulet_tip='2' and gameid='".$key."'");

$kontrol2 = sed_sql_query("select * from kupon_rulet where session_id='$session_id' and rulet_tip='2'");
...
$kontrol = sed_sql_query("select * from kupon_rulet where session_id='$session_id' and secenek='$verigir' and sayi='$sayi' and gameid='$isim_ver' and roundid='$isim_ver2' and rulet_tip='2'");

if(sed_sql_numrows($kontrol)<1) {

sed_sql_query("insert into kupon_rulet (id,session_id,gameid,roundid,oran,secenek,sayi,grupid,rulet_tip,ilkgiris) values ('','$session_id','$isim_ver','$isim_ver2','$oran','$verigir','$sayi','$tiklanansayi','2','$suan')");

} else {

$kupondaki = sed_sql_fetchassoc($kontrol);

sed_sql_query("update kupon_rulet set grupid=grupid+$tiklanansayi where id='$kupondaki[id]' and rulet_tip='2'");
...
$topyatan = 0;
$tutacak_bakiye = 0;
$toplambilgileri = sed_sql_query("select oran,grupid from kupon_rulet where session_id='$session_id' and rulet_tip='2'");
...
$toplamyatirilmisseans = bilgi("SELECT SUM(CASE WHEN id!='' THEN grupid END) as toplamyatan FROM kupon_ic_rulet WHERE oddid='$run_id' and user_id='$ub[id]' and rulet_tip='2'");
...
$sor = sed_sql_query("select * from kupon_rulet where session_id='$session_id' and rulet_tip='2'");

while($row=sed_sql_fetcharray($sor)) {

sed_sql_query("insert into kupon_ic_rulet (id,rulet_tip,oran,session_id,kupon_id,user_id,username,ilkgiris,gameid,roundid,oddid,grupid,secenek,sayi) values ('','2','$row[oran]','$session_id','$kupon_id','$ub[id]','$ub[username]','$row[ilkgiris]','$row[gameid]','$row[roundid]','$run_id','$row[grupid]','$row[secenek]','$row[sayi]')");
...
function bilgigetir() {
$url='https://betwingo.xyz/api/rulet_bilgi.php?cmd=Rulet2';
$ch = curl_init();
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_USERAGENT, "Firefox/3.6.8");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
