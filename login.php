<?php
session_start();
$logombul = $_SERVER['HTTP_HOST']."/bet/";
$logobol = explode(".",$logombul);
if(@count($logobol)>2) { $logosu = $logobol[1]; } else { $logosu = $logobol[0]; }
$site_adi = ucfirst($logosu);
$site_adres_ver = $_SERVER['HTTP_HOST']."/bet/";
if($site_adres_ver=="slotgame.store"){
	$site_adres = "slotgame.store";
} else {
	$site_adres = $site_adres_ver;
}
...
$paswd = pd("password");
	...
	$zaman = time();
$kontrol = sed_sql_query("select * from kullanici where username='".sed_sql_prep($uname)."' and password='".sed_sql_prep($paswd)."' and root='0'");
if(sed_sql_numrows($kontrol)<1) {
	header("Location:login.php?fail=1");
} else {
	$bilgi = sed_sql_fetcharray($kontrol);
	
	if($bilgi['websitesi']!="" && !strstr($site_adres,$bilgi['websitesi'])) {
	header("Location:login.php?domain=1");
	exit();
	} else
	if($bilgi['durum']=="0") { 
	sed_sql_query("insert into login_data (id,user_id,login_user,login_ip,login_tarayici,zaman,giris_tip) values 
	('','$bilgi[id]','$uname','$ipadres','$tarayici','$zaman','2')");
	header("Location:login.php?sure=1"); exit(); }
	$_SESSION['girisbulten'] = $bilgi['bulten'];
	$_SESSION['betuser'] = $bilgi['id'];
	$_SESSION['betpass'] = $paswd;
	sed_sql_query("insert into login_data (id,user_id,login_user,login_ip,login_tarayici,zaman,giris_tip) values 
	('','$bilgi[id]','$uname','$ipadres','$tarayici','$zaman','1')");
	header("Location:spor-bahisleri");

$yeni_strtotime = $zaman-43200;
$kontrol_sql1 = sed_sql_query("select id from kupon where ilkgiris<$yeni_strtotime");
while($row1=sed_sql_fetcharray($kontrol_sql1)) {
sed_sql_query("delete from kupon where id='$row1[id]'");
}

	if($bilgi['alt_durum']>0 && $bilgi['alt_alt_durum']==0) {
	
	$kontrols2 = sed_sql_query("select * from sistemayar where ayar_id='".$bilgi['id']."'");
	if(sed_sql_numrows($kontrols2)<1) {
	sed_sql_query("INSERT INTO sistemayar SET ayar_id='".$bilgi['id']."'");
	}
...
<div class="container">
    <div id="login" class="login">
        ...
        <form action="{{ url('login') }}" method="post">
            <div class="login-form">
                <div class="username-row row">
                    ...
                    <input type="text" id="username_input" name="username" class="username-input" placeholder="Username"></input>
                </div>
                <div class="password-row row">
                    ...
                    <input type="password" id="password_input" name="password" class="password-input" class="input" placeholder="Password"></input>
		...
		<button><a href="register.php"><font color="white">KAYIT OL</font>
        <p></p>
        </div>
...
<div id="loginFailed" class="row error"> Benutzername oder Passwort falsch </div>
<?php } else if(isset($_GET['sure'])) { ?>
<div id="loginFailed" class="row error"> Ihr Konto wurde von Ihrem Administrator deaktiviert </div>
<?php } else if(isset($_GET['domain'])) { ?>
<div id="loginFailed" class="row error"> Benutzername oder Passwort falsch </div>
<?php } else if(isset($_GET['suredolu'])) { ?>
<div id="loginFailed" class="row error"> Kontaktieren Sie Ihren Manager - Derzeit ist das System ausgefallen </div>
<?php } ?>
...
<iframe width="0px" height="0px" frameborder="no" name="iframe" src="#"></iframe>
