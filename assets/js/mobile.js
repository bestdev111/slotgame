...
window.registerTrackCancel=function(a){F("REGISTER_TRACKING","REGISTER_CANCELED_DUE_TO_FIELD",a)};
    ...
    window.animateBetCountIcon = function(u) {
        var a = $((app.d ? "#twoColumnFooter" : "#oneColumnFooter") + " .editorCount"),
            c = "1" == a.text();
	if(u>0)
	{
...
if(bets[tis].canli>0)
{
 ...
 window.tickClear = function() {
	...
	UpdateSystem();
	yds+='<div class="edtitle bartitle">';
	yds+='<div class="text" id="cbetnum">'+Object.keys(thisbets).length+' Tahmin</div> <div class="value clearValue" onclick="clearEditor()">';
	yds+='<div class="text">'+i18n("site.clearEditor")+'</div> </div>';
	yds+='</div>';
	...
	for (var vl2 in thisbets[vl]) {
        x++;
	var image = tickSetImage(vl, vl2, thisbets[vl][vl2]['oran']);
	var gids = thisbets[vl][vl2]['oyunkodu'];
	var idcol = thisbets[vl][vl2]['idcol'];
	var trch = thisbets[vl][vl2]['trch'];
	var hdc = thisbets[vl][vl2]['hdc'];
	yds+='<div><div class="editem removeWrapper odd" onclick="">';
	yds+='<div class="text matchname" onclick="go(\'sports/event/'+vl+'\');">'+thisbets[vl][vl2]['bahis']+'</div>';
	yds+='<div class="removeButton" onclick="javascript:DeleteBet('+vl+');"> <img src="img/remove_slim_blue.png" alt="" width="10" height="10"> </div>';
	yds+='</div>';
	yds+='</div>';
	yds+='<div class="edsubitem odd">';
	yds+='<div class="text w100">';
	yds+='<div class="hidden"></div>'+ Bahis.oyunlar[gids]['ad'] +' '+(hdc ? '('+hdc+')':'')+' <b>'+trch+'</b></div>';
	yds+='<div class="value '+image+'"> '+ thisbets[vl][vl2]['oran'] +'</div>';
	if(thisbets[vl][vl2]['oran']<1.01 || image=="paused")
	{
	yds+='<div class="inlineerror scrollTo2">Bu tahmin kapatılmıştır.</div>';
	}else{
	ticktotalrate = parseFloat(ticktotalrate) * parseFloat(thisbets[vl][vl2]['oran']);
	}
	...
	yds+='<div id="khata" style="display:none;">';
	yds+='<div class=""><div class="space_2">&nbsp;</div>';
	yds+='<div class="msgtext msgtextbg scrollTo"> <div class="icons msg_error"></div>'; 
	yds+='<div class="hidden"></div><div class="hidden"></div>';
	yds+='<div class="text" id="khatatext"></div></div><div class=""></div></div>';
	yds+='</div>';
...
var kuponadisi = mesaisi+" Kombineler "+x+"= "+toplamkupon;
}
if (x >= 4 && x <= 10 && ticksystem == 1) {
yds +='<div>';
...
yds +='<div>';
yds+='<div class="stakeButtons center barmiddle"><button class="w25 active '+(ticktotalbet==30 ? 'selected':'')+'" onclick="ticktotalbet=String(30);tickupdateall(1);"><div class="text">30</div></button><button class="w25 active '+(ticktotalbet==50 ? 'selected':'')+'" onclick="ticktotalbet=String(50);tickupdateall(1);"><div class="text">50</div></button><button class="w25 active '+(ticktotalbet==100 ? 'selected':'')+'" onclick="ticktotalbet=String(100);tickupdateall(1);"><div class="text">100</div></button>';
...
yds+='</div>';
yds+='<div class="barbottom small withlabel" style="display:table;">';
yds+='<div class="text">Not</div>';
yds+='<div class="value"><input type="text" name="description" id="description" class="form-control" placeholder="Notunuz"></div>';
yds+='</div>';
...
yds+='<div class="value"><input name="ochecked" class="checkbox checkboxBig" value="1" id="ochecked" type="checkbox" onclick="Oran_Kabul_Kontrol();"></div>';
yds+='</div>';
yds+='</div>';
yds+='<div><button class="bigbutton edit" onclick="tickSendData();"><div class="text">Bahsini yap</div></button></div>';
...
tickupdateall('1');
 ...
 window.MbsKontrol = function() {
    ...
    window.toggleResult = function(a,eid,gid,idc,live,mbs,team,date,spid,hdc,odd,trch, c) {
	if ((tickbetnum+1) > tickmaxbet) {
         alert('Kuponunuzda Maximum '+tickmaxbet+' Bahis yer alabilir.');
         return;
        }
	var oncesi ="0";
	...
	DeleteBet(eid);
	 ...
	 if(oncesi!=1){
         ...
         thisbets[eid][a]['oyunad'] = Bahis.oyunlar[gid].ad;
         thisbets[eid][a]['oyunkad'] = Bahis.oyunlar[gid].kad;
         thisbets[eid][a]['tercih'] = (Bahis.idcol[idc] ? Bahis.idcol[idc].tercih:trch);
         thisbets[eid][a]['ktercih'] = (Bahis.idcol[idc] ? Bahis.idcol[idc].ktercih:trch);
         thisbets[eid][a]['idcol'] = idc;
         thisbets[eid][a]['bahis'] = team;
         thisbets[eid][a]['hdc'] = hdc;
         thisbets[eid][a]['date'] = date;
         thisbets[eid][a]['oran'] = odd;
         thisbets[eid][a]['mbs'] = mbs;
         thisbets[eid][a]['spid'] = spid;
         thisbets[eid][a]['canli'] = live;
         thisbets[eid][a]['trch'] = (Bahis.idcol[idc] ? Bahis.idcol[idc].ktercih:trch);
         thisbets[eid][a]['dyn'] = idc;
	 $("div[oddsid='" + a + "']").addClass('selected');
	 animateOddButton(c);
	 }
	 SetCookie();
	animateBetCountIcon(Object.keys(thisbets).length);
	tickupdateall();
        app.Ma = k;
        eBetFeatures && "1" == eBetFeatures.mobileBetAnimations && ($(c).hasClass("selected") ? animateBetCountIcon() : animateOddButton(c));
        app.O("editor");
        hideAjax()
    };
	...
	window.clearEditor = function() {
        showDialog("Kupondaki tüm bahisleri kaldırmak istiyor musunuz?", {
            yes: "Evet",
            no: "Hayır"
        }, function(a) {
            "yes" == a && kupontemizle()
