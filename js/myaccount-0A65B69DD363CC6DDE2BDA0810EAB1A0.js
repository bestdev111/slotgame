var ebetAccount = {
    ...
    getCancelWithdrawalUrl: function() {
        return jQuery('#comp-accountPayoutList').attr('e:url');
    },
    ...
    delSelection: function(daysBack, layerTitle, layerTitle2, layerTxt, cancelBtnTxt, confBtnTxt) {
        ...
        showConfLayer(layerTitle, layerTitle2, layerTxt, cancelBtnTxt, confBtnTxt, false, false)
            .then(function(val) {
                if (val) {
                    showDelayLayer();
    ...
    cancelPayment: function(paymentId, layerTitle, layerTitle2, layerTxt, cancelBtnTxt, confBtnTxt) {
        var url = this.getMyContentUrl();
        if (url === undefined)
            url = this.getCancelWithdrawalUrl();
        showConfLayer(layerTitle, layerTitle2, layerTxt, cancelBtnTxt, confBtnTxt, false, false)
            .then(function(val) {
                if (val) {
                    showDelayLayer();
    ...
    cancelTicket: function(ticketId, ticketStatus, layerTitle, layerTitle2, layerTxt, cancelBtnTxt, confBtnTxt) {
        var url = this.getMyContentUrl();
        var paramList = this.getParamListFromUrl(url);
        showConfLayer(layerTitle, layerTitle2, layerTxt, cancelBtnTxt, confBtnTxt, false, true)
            .then(function(val) {
                if (val) {
                    showDelayLayer();
    ...
    updateDelRequest: function(delRequest) {
        //			newParamList = this.getParamListFromUrl(this.getMyContentUrl()).replace("delRequest=" + delRequest, "delRequest=" + !delRequest);
