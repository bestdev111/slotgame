...
var casibostema ='<div class="coupon-empty"><strong>Kuponunuz boş.</strong><span class="text-muted">Oyun eklemek için oranlara tıklayınız.</span></div>';
var casinobetnum = 0;
...
function get_casino_bets() {
	...
	if ([5,6,8].indexOf(_active_game) === -1) {
		...
		$('.casino-holder').prepend('<iframe src="https://href.li/?https://webiframe.betgames.tv/#/auth?apiUrl=https%3A%2F%2Fgame3.betgames.tv&partnerCode=olybet&partnerToken=-&language=tr&timezone=3&pageName='+_active_game+'" allowtransparency="true" frameborder="0" scrolling="no" onload="$(\'.nest5-wrapper\').hide(); $(this).css(\'opacity\', 1); casino_bet_ajax();" style="opacity: 0;"></iframe>');
...
var extraklubs="";
	$.ajax({
		...
		success: function(json) {
			...
			$tabs = '<div class="casino-tabs-holder"><div class="casino-tabs"><div class="casino-tab '+(_active_tab == 0 ? 'active' : '')+'"  id="tab_0"  onclick="changetab(0);" data-id="0"><span>Tümü</span></div>'
			$.each(json.game.bets,function(key, data) {
				$html = '<div class="casino-tab '+(_active_tab == data.id ? 'active' : '')+'" data-id="'+data.id+'" id="tab_'+data.id+'" onclick="changetab('+data.id+');" ><span>'+_text.oddGroups[data.id]+'</span></div>';
				$tabs = $tabs + $html;
				...
				$.each(data.odds, function(key, odds) {
					...
					$html = $html + '<div data-itemcount="'+odds.itemsCount+'" id="thegame_'+odds.id+'" class="casino-bet '+(odds.itemsCount > 0 ? 'openable' : '')+'" data-odds="'+odds.value.toFixed(2)+'" data-gameid="'+_active_game+'" data-id="'+odds.id+'" data-grupid="'+data.id+'" data-roundid="'+json.game.run_id+'" '+(odds.itemsCount > 0 ? '' : 'onclick="casinoaddselect(this);"')+' ><div class="casino-bet-top" '+(odds.itemsCount > 0 ? 'onclick="casinoaddopenwon('+odds.id+');"' : '')+'><span class="casino-bet-title">'+_text.odds[odds.id]+''+extraklubs+'</span>'
...
function casino_refresh() {
	...
	$.ajax({
		...
		success: function(json) {
			...
			$('#casinodiv_'+data.id).html('<i style="color: red;font-size: 9px;">CANLI</i>');
			}
...
if(casinobetnum>0)
{
             ...
             $htmlcasino.push(''+casinobetsin.gname+' <span class="badge badge-dark text-danger bet-timeleft">'+(casinobetsin.timeleftgame!=0 && casinobetsin.timeleftgame!="" ? casinobetsin.timeleftgame:'CANLI')+'</span>');
             $htmlcasino.push('</span>');
             $htmlcasino.push('</div>');
             $htmlcasino.push('<div class="cp-bet">');
             $htmlcasino.push('<span class="bet-name">'+casinobetsin.odname+'</span>');
		...
		{
             $htmlcasino.push('<div class="mbs-error">Bu bahsi eklediğiniz seans ('+casinobetsin.oldround+') kapanmıştır, kuponunuz '+casinobetsin.round+' seansına oynanacaktır.</div>');
		}
             $htmlcasino.push('<div class="coupon-fresh">');
             $htmlcasino.push('<span class="coupon-count">Toplam 1 müsabaka</span>');
             $htmlcasino.push('<span class="delete-coupon btn btn-sm btn-success" data-type="casino" onclick="celarcasinocoupon();">Kuponu Temizle</span>');
             $htmlcasino.push('</div>');
             $htmlcasino.push('<div class="slip-alt">');
             $htmlcasino.push('<ul class="slip-input">');
             $htmlcasino.push('<li>');
             $htmlcasino.push('<span>Kupon Notu:</span>');
             $htmlcasino.push('<span><input type="text" name="title" id="kuponisim" value=""></span>');
             $htmlcasino.push('</li>');
             $htmlcasino.push('<li>');
             $htmlcasino.push('<span>Yatırılan:</span>');
             $htmlcasino.push('<span>');
             ...
             $htmlcasino.push('</span>');
             $htmlcasino.push('</li>');
             $htmlcasino.push('<li><span>Olası Kazanç:</span><span><span id="max-gain">'+totalcasinobet+'</span></span></li>');
             $htmlcasino.push('</ul>');
             $htmlcasino.push('<div class="cp-button">');
             $htmlcasino.push('<input type="hidden" name="coupon_type" value="casino">');
             $htmlcasino.push('<button type="button" data-qa="button-place-bet" class="btn btn-mini btn-info" onclick="sendcasinocoupon();" style="width: 95%;height: 30px !important;">Bahisi onayla</button>');
             $htmlcasino.push('</div>');
             $htmlcasino.push('</div>');
...
if (!casinobetnum) {
casPopupError('Kuponunuzda oyun bulunmamaktadır');
return;
}

if (max_amount<totalcasinobet) {
casPopupError('Max. '+max_amount+' Miktarda Bahis Yapabilirsiniz.');
return;
}

if (min_amount>totalcasinobet) {
casPopupError('Min. '+min_amount+' Miktarda Bahis Yapabilirsiniz.');
return;
}
...
if(data=="403") {
casPopupError('Bakiyeniz Yetersizdir.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="404") {
casPopupError('Başlamış oyuna oynayamazsınız.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="410") {
casPopupError('Basmaca - Max Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="411") {
casPopupError('Poker - Max Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="412") {
casPopupError('Bakara - Max Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="413") {
casPopupError('Çarkıfelek - Max Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="414") {
casPopupError('Zar Düellosu - Max Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="415") {
casPopupError('6+ Poker - Max Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="416") {
casPopupError('Sayısal Loto 5 - Max Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="417") {
casPopupError('Sayısal Loto 6 - Max Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="418") {
casPopupError('Sayısal Loto 7 - Max Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="420") {
casPopupError('Basmaca - Min Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="421") {
casPopupError('Poker - Min Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="422") {
casPopupError('Bakara - Min Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="423") {
casPopupError('Çarkıfelek - Min Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="424") {
casPopupError('Zar Düellosu - Min Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="425") {
casPopupError('6+ Poker - Min Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="426") {
casPopupError('Sayısal Loto 5 - Min Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="427") {
casPopupError('Sayısal Loto 6 - Min Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="428") {
casPopupError('Sayısal Loto 7 - Min Bahis(Tutar) Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data=="430") {
casPopupError('Oran Hatası.');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
} else if(data!="404") {
celarcasinocoupon();
$('.betting-slip-content').html('<div class="notification-container" style="opacity: 1;width: 93%;"><div class="notification notification-success"><span class="notification-icon betgames-icon check"></span><span class="notification-message" data-qa="text-bet-slip-notification">Bahis kabul edildi.</span></div></div>');
setTimeout(function(){ casinocouponloader(); }, 5000);
unblock($('#coupon'));
casinosend = false;
_lottery_items = '';
