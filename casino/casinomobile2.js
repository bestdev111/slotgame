...
var casibostema =' <div class="betgames-icon betslip-empty"></div><div><div class="bet-slip-empty-title">Bahis kuponu boş</div><div class="bet-slip-empty-info">Listeden bahis seçiniz</div></div><footer><div class="bet-slip-form" id="betslipalert"></div></footer>';
var casinobetnum = 0;
		...
		success: function(json) {
			...
			$.each(json.game.bets,function(key, data) {
			  ...
			  _active_tab=data.id;
				...
				$html = '<div class="casino-tab tabs-bar-item '+(_active_tab == data.id ? 'active' : '')+'" data-id="'+data.id+'" id="tab_'+data.id+'" onclick="changetab('+data.id+');" ><span>'+_text.oddGroups[data.id]+'</span></div>';
				$tabs = $tabs + $html;
...
$(".betting-slip-content").removeClass('bet-slip-empty');
		...
		$htmlcasino.push('<div class="selected-odd-info"><span data-qa="text-selected-odd-name">'+casinobetsin.odname+'<span class="cards-group"></span></span>');
		...
		{
                $htmlcasino.push('<div class="mbs-error"><div class="selected-odd-message" data-qa="text-selected-odd-message">'+casinobetsin.oldround+' Seansı kapanmıştır. Bahis sıradaki çekiliş için kabul edilecektir. Sıradaki Seans - '+casinobetsin.round+'</div>');
		}
		$htmlcasino.push('<div class="bet-slip-form" id="betslipalert"></div>');
		$htmlcasino.push('<div class="bet-slip-form">');
		$htmlcasino.push('<div class="bet-slip-limits"><span>Kupon Notu</span></div>');
		...
		$htmlcasino.push('<div class="bet-slip-limits"><span>Miktar</span></div>');
		...
		$htmlcasino.push('</div>');
		$htmlcasino.push('<div class="_2qtcGvjc33NQUNf6zumhCo"><span data-qa="label-possible-win">Olası kazanç</span>');
		$htmlcasino.push('<span data-qa="text-bet-slip-possible-win" id="max-gain">'+totalcasinowin+'</span></div><button type="button" data-qa="button-place-bet"  onclick="sendcasinocoupon();" id="bahisionayla" class="place-bet-button">Bahisi onayla</button>');
		$htmlcasino.push('</footer>');
...
$('#bahisionayla').html('<font style="animation: hello 1.5s ease-in-out infinite;font-weight:bold">Onaylanıyor...</font>');
if (casinosend) {
return;
}
