...
<tbody><tr>
<th class="l"><? if(getTranslation('basketboloran'.$tip['id'].'')!=''){ ?><?=getTranslation('basketboloran'.$tip['id'].'')?><? } else { ?><?=$tip['tip_isim'];?><? } ?></th>
...
<?
$sss = sed_sql_query("select id,oran_val_b,mac_db_id,oran_tip,oran_val_id,oran,durum from oranlar_basketbol ora where mac_db_id='$mac_id' and durum='1' and oran_tip='$ass[oran_tip]' $gizli_ekle group by oran_val_id,oran_val_b order by id asc,oran_val_id asc");
...
$buoran = $row['oran'];
$oran_bilgi_2 = bilgi("select oran from maclar_oranlar where oran_val_id='".$row['id']."' and mac_db_id='".$mac_id."' and spor_tipi='basketbol' and bayi_id='".$ub['id']."'");
...
<a href="javascript:;" class="tag tag-danger dealer-increase-limit" onclick="odchangeminus(<?=$row['id']; ?>);"><i class="fa fa-level-down"></i> -0.05</a>
<a href="javascript:;" class="tag tag-success dealer-increase-limit" onclick="odchangeplus(<?=$row['id']; ?>);"><i class="fa fa-level-up"></i> +0.05</a>

</td>

<td style="text-align:center;"><input readonly="readonly" class="inputText" id="chod_<?=$row['id']; ?>" type="text" name="rid[<?=$row['id']; ?>]" style="width: 70px" size="50" maxlength="8" value="<?if($oran_bilgi_2['oran']!=''){ ?><?=$oran_bilgi_2['oran'];?><? } else { ?>0.00<? } ?>"></td>
