...
<li class="breadcrumb-item"><?=getTranslation('yeniyerler_kasa110')?></li>
</ol>
...
var sure = confirm('<?=getTranslation('yeniyerler_kasa111')?>');
if (!sure) return sure
$.get('../ajax_superadmin.php?a=casinoremoveamount&id='+cs, function(result){
$('#info').show().html('<?=getTranslation('yeniyerler_kasa112')?>');
if (result=="101") {
$('#info').hide();
$('#success').show().html('<?=getTranslation('yeniyerler_kasa113')?>');
$('#error').hide();
} else if (result=="202") {
$('#info').hide();
$('#success').hide();
$('#error').show().html('<?=getTranslation('yeniyerler_kasa114')?>');
}
});
}
...
$.get('../ajax_superadmin.php?a=casinohesaprapor&rand='+rand+'&tarih1='+tarih1+'&tarih2='+tarih2+'',function(data) { $("#kupons").html(data); });
}
